...
const Layout: React.FC<{ props: Object }> = (props) => {
  ...
  useEffect(() => {
    ...
    const currentIndex =
      pathname !== "/[category]" && pathname === "/"
        ? categoriesData.indexOf("todos")
        : pathname === "/completed"
        ? categoriesData.length
        : categoriesData.indexOf(currentPath);
  ...
  const formSubmitHandler = (e: React.FormEvent) => {
    ...
    const categoryTitle: string | undefined =
      categoryInputRef.current?.value.toLocaleLowerCase();
    const todo: TodoType = {
      id: 0,
  ...
  return (
    <main className="app">
      <nav className={styles.nav}>
        ...
        >
          {(state) => (
            <CloseRounded
              style={{
                transition: "all .3s",
                opacity: state === "exiting" ? 0.1 : 1,
                transform: state === "exiting" ? "rotate(90deg)" : "rotate(0)",
              }}
              className={classes.navIcon}
              onClick={() => {
                setDrawerVisibility((drawer) => !drawer);
              }}
            />
          )}
        </ReactTransition>
        ...
        <Typography
          className={`${styles.heading} ${classes.colorWhite}`}
          variant="h4"
        >
          {pathname === "/" ? (
            <React.Fragment>
      ...
      <div className={styles.todoContainer}>{props.children}</div>
        ...
        >
          <div
            className={`${styles.drawer} ${
              drawerVisibility ? styles.drawerActive : ""
            }`}
          >
            <List
              className={`${styles.drawerInnerContainer} ${classes.drawerInnerContainer}`}
              component="nav"
            >
              {categoriesData.map((category, i) => {
                return (
                  <div key={category} className={styles.categoryContainer}>
                    ...
                    <Button
                      onClick={() => {
                        showDeleteConfirmationHandler(category);
                      }}
                ...
                >
                  <div className={styles.categoryFormContainer}>
                    <ListItemButton
                      ...
                      onClick={(event) =>
                        handleListItemClick(event, categoriesData.length)
                      } //index
                    >
                      <form
                      ...
                      >
                        ...
                        <span className={styles.categoryFormSubmitBtnContainer}>
                          <Button
                            className={classes.categoryFormSubmitBtn}
                            type="submit"
                            variant="outlined"
                            color="primary"
                          >
              ...
              <Link href="/completed">
                <ListItemButton
                  ...
                  onClick={(event) =>
                    handleListItemClick(event, categoriesData.length + 1)
                  }
                >
                  <ListItemIcon>
            ...
            <Button
              ...
              className={`${styles.addCategoryBtn} ${classes.addMoreCategoryBtn}`}
              color="primary"
              onClick={addCategoryHandler}
            >
              <Typography
                noWrap
                className={classes.addCategoryHeading}
                variant="h6"
              >
                Add More Categories
              </Typography>
            ...
            {showDeleteConfirmation && (
              <div className={styles.deleteCategoryConfirmationContainer}>
                ...
                >
                  <div
                    className={styles.deleteCategoryConfirmationInnerContainer}
                  >
                    <Typography variant="h6" color="primary">
                      Are you sure you want to delete{" "}
                      <span
                        className={`${classes.deleteCategoryName} ${styles.deleteCategoryName}`}
                      >
                        {deleteCategoryName}
                      </span>{" "}
                      category!
